...
function GuardarServer(){
    ...
    if (ValidarDatos(nombre,apellido)){
        ...
        $.ajax({
            ...
            dataType: "text",
            data: persona,
            success:gestionarRespuestaGuardarServer(respuesta),
            cache: false,
...
function TraerPersonas(){
    ...
    $.ajax({
        ...
        type: "get",
        dataType: "html",
        success:gestionarRespuestaTraerServer(respuesta),
        cache: false,
        error:function(jqXHR, textStatus, textError){ alert("error!!" + textStatus + textError)},
        contentType: false,
        processData: false         
...
function gestionarRespuestaTraerPersonas(respuesta) {

    var div = $('#respuesta');

            //Si esta todo bien, guardo la respuesta del server
            var objPersonas = JSON.parse(respuesta);
   ...
   LimpiarTabla();
    ...
    for(i = 0; i < objPersonas.length; i++ ){
        ...
        tCuerpo.innerHTML =   tCuerpo.innerHTML +
            "<tr><td>" + objPersonas[i].nombre + "</td><td>" 
            + objPersonas[i].apellido + 
            "</td><td><input id='btnModificar'  type='button' value='Modificar' onclick='TraerPersona(" + i + ")'>" + "</td><td><input id='btnEliminar'  type='button' value='Eliminar' onclick='EliminarPersona(" + i + ")'></td></tr>"; 
    }
}
...
{
    //Boton guardar
    var btnGuardar = $("#btnGuardar").click(GuardarServer());
...
if(nombre=='' && apellido=='')
    {  
        var nombre = $('#txtNombre').style.borderColor="red";
        var apellido = $('#txtApellido').style.borderColor="red";  
        return false;
    } else if(nombre=='')
    {
        var nombre = $('#txtNombre').style.borderColor="red";
